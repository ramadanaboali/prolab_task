...
@section('content')
    ...
    <div class="row pt-2 pb-2">
        <div class="col-sm-9">
            <h4 class="page-title">{{__('app.partners')}}</h4>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('home')}}">{{__('app.dashboard')}}</a></li>
                <li class="breadcrumb-item"><a href="{{ route('partners.index') }}">{{ __('app.partners') }}</a></li>
            </ol>
        </div>
                    ...
                    <div class="table-responsive">
                        <table id="example" class="table table-bordered">
                            ...
                            <tr>
                                ...
                                <th>{{__('app.crud.table.url')}}</th>
                                <th>{{__('app.crud.table.status')}}</th>
                                <th>{{__('app.crud.table.photo')}}</th>
                            ...
                            @foreach($data as $row)
                                <tr id="row_{{$row->id}}">
                                    ...
                                    <td>
                                        ...
                                        <a href="{{asset($row->logo)}}" data-fancybox="group2">
                                            <img width="75px" height="75px" src="{{asset($row->logo)}}" alt="{{$row->name}}" class="">
                                        </a>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <button type="button" class="btn btn-danger waves-effect waves-light btn-sm" onclick="delete_alert({{$row->id}},'partners')" ><i class="fa fa-trash" aria-hidden="true"></i></button>
                                    </td>
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content animated swing">
                                                ...
                                                <div class="modal-body container">
                                                    <form method="POST" action="{{route('partners.update',[$row->id])}}" id="update_form_{{$row->id}}"  enctype="multipart/form-data">
                                                        @csrf
                                                        ...
                                                        <div class="col-md-12">
                                                            <div class="form-group">
                                                                <label>{{__('app.crud.table.url')}}</label>
                                                                <input type="text" id="url" class="form-control input-shadow @error('url') is-invalid @enderror" value="{{$row->url}}" name="url" placeholder="{{__('app.crud.table.url')}}" required>
                                                                @error('url')
                                                                <span class="invalid-feedback" role="alert">
                                                                    <strong>{{ $message }}</strong>
                                                                </span>
                                                        ...
                                                        <div class="col-md-12">
                                                            <div class="form-group">
                                                                <label>{{__('app.crud.table.photo')}}</label>
                                                                <input  onchange="loadFile(event)" type="file" class="form-control input-shadow @error('photo') is-invalid @enderror" name="photo" placeholder="{{__('app.crud.table.photo')}}">
                                                                @error('photo')
                                                                <span class="invalid-feedback" role="alert">
                                                                        <strong>{{ $message }}</strong>
                                                                    </span>
                                                        ...
                                                        <div class="col-md-12">
                                                            <div class="form-group">
                                                                @if (!empty($row->logo))
                                                                <img id="output" src="{{asset($row->logo)}}" width="120px" height="120px">
                                                                @endif
                                                            </div>
                                                        </div>
                ...
                <div class="modal-body container">
                    <form action="{{route('partners.store')}}"  method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <label>{{__('app.crud.table.url')}}</label>
                                <input type="text" class="form-control input-shadow @error('url') is-invalid @enderror" name="url" placeholder="{{__('app.crud.table.url')}}" required>
                                @error('url')
                                <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <label>{{__('app.crud.table.photo')}}</label>
                                <input type="file" class="form-control input-shadow @error('photo') is-invalid @enderror" name="photo" size="100">
                                @error('photo')
                                <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
