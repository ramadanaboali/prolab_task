...
@section('content')
    ...
    <div class="row pt-2 pb-2">
        <div class="col-sm-9">
            <h4 class="page-title">{{__('app.notifications')}}</h4>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{route('home')}}">{{__('app.dashboard')}}</a></li>
                <li class="breadcrumb-item"><a href="">{{__('app.notifications')}}</a></li>
            </ol>
        </div>
                        ...
                        <table id="example" class="table table-bordered">
                            ...
                            @foreach($data as $row)
                                <tr id="row_{{$row->id}}">
                                    ...
                                    <td>{{$row->message}}</td>
                                    <td>{{$row->user->name}}</td>
                                    <td>{{$row->sender != null ? $row->sender->name: ''}}</td>
